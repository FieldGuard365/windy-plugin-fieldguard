...
<div class="fg">
  ...
  <div class="fg-head">
    ...
    <div class="fg-head-txt">
      <span class="fg-head-title">FieldGuard</span>
      <span class="fg-head-sub">Real-time Heat & Weather Safety</span>
    </div>
    <button class="fg-icon-btn" on:click={refreshData} title="Refresh">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" width="14" height="14"><path d="M3 12a9 9 0 0 1 9-9 9.75 9.75 0 0 1 6.74 2.74L21 8"/><path d="M21 3v5h-5"/><path d="M21 12a9 9 0 0 1-9 9 9.75 9.75 0 0 1-6.74-2.74L3 16"/><path d="M8 16H3v5"/></svg>
    </button>
  </div>
  ...
  <!-- ══ DESKTOP DETAIL — hidden on mobile via #device-mobile ══ -->
  <div class="fg-desktop">
    ...
    {#if tab === 'dashboard'}
      {#if loading}
        <div class="fg-load"><div class="fg-spin"></div> Loading {worstCaseMode?'all models':selectedModel}…</div>
      {:else if error}
        <div class="fg-err">{error}</div>
      {:else if heat}
        {#if heat.isBanPeriod}
        ...
        {:else if activeCard === 'solar'}
          <div class="fg-det" style="border-color:{solarColor}">
            ...
            {#if isNight}
              <div class="fg-night">🌙 Zero solar at night<br/><small style="color:#4a6090">Sunrise: {sunriseTime} · Sunset: {sunsetTime}</small></div>
            {:else}
              <div class="fg-det-g">
        ...
        {:else}
          <div class="fg-det-hint">↑ Select a card above</div>
        {/if}
        <div class="fg-mr">
    ...
    {:else if tab === 'sos'}
      {#if !license.valid}
        <div class="fg-gate"><div class="fg-gate-ic">🚨</div><div class="fg-gate-ti">SOS — Pro Feature</div><a class="fg-gate-btn" href="https://fieldguard-hse.com" target="_blank">Upgrade</a></div>
      {:else}
        <div class="fg-emg">
    ...
    {:else if tab === 'report'}
      {#if !license.valid}
        <div class="fg-gate"><div class="fg-gate-ic">📄</div><div class="fg-gate-ti">Reports — Pro Feature</div><a class="fg-gate-btn" href="https://fieldguard-hse.com" target="_blank">Upgrade</a></div>
      {:else}
        <div class="fg-rform">
    ...
    {:else if tab === 'settings'}
      <div class="fg-grp">
        ...
        {:else}
          <div class="fg-lic-free"><div class="fg-lic-ft">FREE TIER</div><div class="fg-lic-fl">⚡ Worst-case · 📄 Reports · 🚨 SOS · 🎛 Thresholds</div><a class="fg-btn-amb" href="https://fieldguard-hse.com" target="_blank">Get Pro — fieldguard-hse.com</a></div>
...
<style>
  /* FieldGuard v3.3
     ...
     Desktop: full right panel with header + pills + detail
     Mobile:  pills row only (fg-desktop hidden via #device-mobile)
  */
  :root {
    --amb:#e8962a; --n1:#050a18; --n2:#0a1228; --n4:#1a2d55;
  ...
  .fg-head-logo  { width: 26px; height: 26px; object-fit: contain; flex-shrink: 0; }
  .fg-head-shield{ font-size: 18px; flex-shrink: 0; }
  .fg-head-txt   { flex: 1; display: flex; flex-direction: column; }
  .fg-head-title { font-size: 13px; font-weight: 800; color: #fff; }
  .fg-head-sub   { font-size: 8px; color: var(--sl2); text-transform: uppercase; letter-spacing: 0.8px; }
  .fg-icon-btn {
    width: 26px; height: 26px; background: rgba(15,29,66,0.8);
    border: 1px solid rgba(45,64,128,0.6); border-radius: 6px;
    display: flex; align-items: center; justify-content: center;
    cursor: pointer; color: var(--sl); transition: all 0.15s; flex-shrink: 0; padding: 0;
  }
  .fg-icon-btn:hover { border-color: var(--amb); color: var(--amb); }

  /* ── LOCATION BAR ─────────────────────────────────────────── */
  .fg-loc { display: flex; align-items: center; gap: 4px; padding: 4px 10px; background: rgba(10,18,40,0.8); border-bottom: 1px solid rgba(255,255,255,0.04); font-size: 9px; color: var(--sl); }
  .fg-loc-name { flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
  .fg-loc-r    { display: flex; align-items: center; gap: 4px; flex-shrink: 0; }
  .fg-loc-t    { color: var(--sl2); }
  .fg-dbadge   { background: #78350f; color: #fcd34d; border-radius: 3px; padding: 1px 5px; font-size: 7px; font-weight: 700; }
  .fg-nbadge   { background: #1e1b4b; color: #a5b4fc; border-radius: 3px; padding: 1px 5px; font-size: 7px; font-weight: 700; }

  /* ── PILL ROW — shown on ALL platforms ───────────────────── */
  .fg-pills {
    display: flex; align-items: stretch; gap: 5px;
    padding: 8px 8px 6px;
    background: #0d1933;
    border-bottom: 1px solid rgba(232,150,42,0.2);
  }
  .fg-pill {
    flex: 1; min-width: 0; display: flex; flex-direction: column;
    align-items: center; gap: 2px; padding: 8px 3px;
  ...
  .fg-pill-ic { font-size: 16px; line-height: 1; }
  .fg-pill-vl { font-size: 14px; font-weight: 800; color: #fff; line-height: 1.1; display: flex; align-items: baseline; gap: 1px; }
  .fg-pill-u  { font-size: 8px; color: var(--sl); }
  .fg-pill-lb { font-size: 8px; font-weight: 800; text-transform: uppercase; letter-spacing: 0.4px; }
  .fg-pill-ban{ font-size: 18px; align-self: center; flex-shrink: 0; animation: pulse 1.5s infinite; }
  @keyframes pulse { 0%,100%{opacity:1} 50%{opacity:.4} }
  .fg-rfr {
    width: 28px; flex-shrink: 0; align-self: center;
    background: rgba(10,18,40,0.7); border: 1px solid rgba(45,64,128,0.5);
    border-radius: 8px; font-size: 16px; cursor: pointer; color: var(--sl);
    padding: 5px; display: flex; align-items: center; justify-content: center;
    transition: all 0.15s;
  }
  .fg-rfr:hover { border-color: var(--amb); color: var(--amb); }
  .fg-pills-load{ display: flex; align-items: center; gap: 8px; font-size: 11px; color: var(--sl); padding: 6px 2px; }
  .fg-pills-tap { width: 100%; background: transparent; border: 1px dashed rgba(45,64,128,0.5); border-radius: 7px; color: var(--sl); font-size: 11px; cursor: pointer; padding: 8px; }

  /* ── DESKTOP SECTION — hidden on mobile ──────────────────
     Windy sets id="device-mobile" on <html> for mobile/tablet
  ──────────────────────────────────────────────────────────── */
  .fg-desktop { display: block; }
  #device-mobile .fg-desktop { display: none !important; }
  /* Also hide header/loc bar on mobile */
  #device-mobile .fg-head { display: none !important; }
  #device-mobile .fg-loc  { display: none !important; }

  /* ── TABS ─────────────────────────────────────────────────── */
  .fg-tabs { display: flex; background: #0a1228; border-bottom: 1px solid rgba(255,255,255,0.06); }
  .fg-tab  { flex: 1; padding: 6px 2px; background: transparent; border: none; color: var(--sl2); cursor: pointer; font-size: 9px; border-bottom: 2px solid transparent; transition: all 0.15s; display: flex; flex-direction: column; align-items: center; gap: 1px; }
  .fg-tab span:first-child { font-size: 13px; }
  .fg-tab.fg-tab-on { color: var(--amb); border-bottom-color: var(--amb); }

  /* ── DETAIL ───────────────────────────────────────────────── */
  .fg-det { background: rgba(5,10,24,0.8); border: 1px solid; margin: 6px 8px; border-radius: 8px; padding: 10px 9px; }
  .fg-det-ti { font-size: 11px; font-weight: 800; text-transform: uppercase; letter-spacing: 0.4px; margin-bottom: 8px; }
  .fg-det-g  { display: grid; grid-template-columns: repeat(3,1fr); gap: 3px; margin-bottom: 7px; }
  .fg-dc { background: rgba(5,10,22,0.6); border-radius: 5px; padding: 5px 3px; text-align: center; }
  ...
  .fg-ds { background: rgba(5,10,22,0.4); border-radius: 5px; padding: 6px; margin-bottom: 5px; }
  .fg-dr { display: flex; justify-content: space-between; padding: 2px 0; border-bottom: 1px solid rgba(10,18,38,0.9); font-size: 9px; }
  .fg-dr:last-child { border: none; }
  .fg-drl { color: var(--sl2); } .fg-drv { color: var(--sl); font-weight: 500; }
  .fg-dct  { font-size: 9px; color: var(--amb); text-transform: uppercase; font-weight: 700; margin: 6px 0 4px; }
  .fg-dci  { font-size: 9px; color: var(--sl); padding: 2px 0; border-bottom: 1px solid rgba(10,18,38,0.7); }
  .fg-dci:last-child { border: none; }
  .fg-dppe { font-size: 8px; color: var(--sl2); margin-top: 5px; }
  .fg-dthr { font-size: 9px; color: var(--sl2); text-align: center; margin: 4px 8px 0; }
  .fg-night{ background: rgba(30,27,75,0.5); border-radius: 6px; padding: 12px; text-align: center; font-size: 10px; color: #c7d2fe; }
  .fg-det-hint { padding: 18px; text-align: center; color: var(--sl2); font-size: 10px; }
  .fg-ban  { padding: 7px 10px; background: rgba(124,45,18,0.85); color: #fed7aa; font-size: 10px; font-weight: 700; text-align: center; }

  .fg-load { display: flex; align-items: center; gap: 8px; padding: 18px 10px; font-size: 10px; color: var(--sl); }
  .fg-err  { padding: 10px; background: rgba(61,10,10,0.85); color: #fca5a5; font-size: 10px; }

  /* ── MODEL ROW ────────────────────────────────────────────── */
  .fg-mr { display: flex; align-items: center; gap: 6px; padding: 6px 8px; border-top: 1px solid rgba(255,255,255,0.05); background: rgba(10,18,40,0.5); }
  .fg-mr-sel { background: rgba(10,18,40,0.9); border: 1px solid rgba(45,64,128,0.5); color: #e8edf8; padding: 3px 6px; border-radius: 4px; font-size: 10px; }
  .fg-mr-wc  { display: flex; align-items: center; gap: 4px; margin-left: auto; font-size: 9px; color: var(--sl); cursor: pointer; }
  .fg-proch  { background: var(--amb); color: #0f1d42; font-size: 7px; font-weight: 800; padding: 1px 4px; border-radius: 2px; }

  .fg-tbl    { width: 100%; border-collapse: collapse; font-size: 9px; margin: 0 8px 6px; width: calc(100% - 16px); }
  ...
  /* ── SOS ──────────────────────────────────────────────────── */
  .fg-emg    { padding: 10px 10px; }
  .fg-emg-w  { font-size: 11px; font-weight: 700; color: #f87171; margin-bottom: 5px; }
  .fg-emg-syms { display: flex; flex-wrap: wrap; gap: 3px; margin-bottom: 8px; }
  .fg-sym    { background: rgba(61,10,10,0.8); color: #fca5a5; border-radius: 3px; padding: 2px 6px; font-size: 9px; }
  .fg-emg-hd { font-size: 9px; color: var(--sl2); text-transform: uppercase; font-weight: 700; margin-bottom: 5px; }
  .fg-emg-step { display: flex; gap: 7px; align-items: flex-start; padding: 3px 0; font-size: 9px; color: var(--sl); border-bottom: 1px solid rgba(10,18,38,0.8); }
  .fg-emg-step:last-child { border: none; }
  .fg-emg-n  { background: rgba(45,64,128,0.6); color: #e8edf8; border-radius: 3px; padding: 1px 5px; font-size: 8px; font-weight: 700; flex-shrink: 0; }
  .fg-emg-crit { color: #f87171 !important; font-weight: 700; }
  .fg-emg-crit .fg-emg-n { background: #dc2626; }
  .fg-sec    { padding: 8px 10px 3px; font-size: 9px; color: var(--amb); text-transform: uppercase; font-weight: 700; }
  .fg-alog   { margin: 3px 10px; padding: 6px 8px; background: rgba(10,18,40,0.7); border-radius: 5px; border-left: 3px solid; }
  ...
  /* ── REPORT ───────────────────────────────────────────────── */
  .fg-rform  { padding: 7px 10px; display: grid; grid-template-columns: 1fr 1fr; gap: 5px; }
  label      { display: block; color: var(--sl); font-size: 9px; }
  label input, label select { display: block; width: 100%; background: rgba(5,10,22,0.9); border: 1px solid rgba(45,64,128,0.5); color: #e8edf8; padding: 4px 7px; border-radius: 4px; font-size: 9px; margin-top: 2px; box-sizing: border-box; }
  label input:focus, label select:focus { outline: none; border-color: var(--amb); }
  .fg-btn    { display: block; width: 100%; padding: 9px; border: none; font-size: 11px; font-weight: 800; cursor: pointer; background: var(--amb); color: #0f1d42; }
  .fg-btn-sec{ background: rgba(10,18,40,0.8); color: var(--sl); border: 1px solid rgba(45,64,128,0.4); border-radius: 5px; width: calc(100% - 20px); margin: 0 10px; }
  .fg-btn-amb{ display: block; text-align: center; text-decoration: none; background: var(--amb); color: #0f1d42 !important; padding: 8px; border-radius: 5px; font-size: 10px; font-weight: 800; margin-top: 5px; }
  .fg-rep    { overflow: hidden; border-top: 1px solid rgba(45,64,128,0.3); }
  .fg-rep-bar{ display: flex; align-items: center; gap: 6px; padding: 5px 10px; background: rgba(10,18,40,0.8); font-size: 9px; color: var(--sl2); }
  .fg-rep-bar span { flex: 1; }
  .fg-rep-btn{ background: rgba(45,64,128,0.5); border: none; color: var(--sl); padding: 2px 8px; border-radius: 3px; cursor: pointer; font-size: 9px; }
  .fg-rep-txt{ padding: 8px 10px; font-size: 8px; color: var(--sl); white-space: pre; overflow: auto; max-height: 220px; font-family: monospace; line-height: 1.4; }

  /* ── SETTINGS ─────────────────────────────────────────────── */
  .fg-grp    { padding: 9px 10px; border-bottom: 1px solid rgba(45,64,128,0.2); }
  ...
  .fg-lic-b  { background: rgba(5,46,22,0.8); color: #4ade80; border: 1px solid #16a34a; border-radius: 4px; padding: 2px 8px; font-size: 10px; font-weight: 700; }
  ...
  .fg-lic-free { background: rgba(5,10,22,0.6); border-radius: 5px; padding: 9px; margin-bottom: 7px; }
  .fg-lic-ft { font-size: 8px; font-weight: 700; color: var(--sl2); text-transform: uppercase; margin-bottom: 4px; }
  .fg-lic-fl { font-size: 9px; color: var(--sl2); line-height: 1.7; margin-bottom: 5px; }
  .fg-lic-row{ display: flex; gap: 4px; }
  .fg-lic-in { flex: 1; background: rgba(5,10,22,0.9); border: 1px solid rgba(45,64,128,0.5); color: #e8edf8; padding: 4px 7px; border-radius: 4px; font-size: 9px; font-family: monospace; }
  .fg-lic-in:disabled { opacity: 0.5; }
  .fg-lic-ab { background: var(--amb); border: none; color: #0f1d42; padding: 4px 12px; border-radius: 4px; cursor: pointer; font-size: 10px; font-weight: 800; white-space: nowrap; }
  .fg-lic-ab:disabled { background: rgba(45,64,128,0.4); color: var(--sl2); cursor: not-allowed; }
  .fg-lic-err{ margin-top: 5px; font-size: 9px; color: #f87171; padding: 4px 7px; background: rgba(61,10,10,0.8); border-radius: 4px; }
  ...
  /* ── GATE ─────────────────────────────────────────────────── */
  .fg-gate   { padding: 16px 12px; text-align: center; }
  .fg-gate-ic{ font-size: 28px; margin-bottom: 6px; }
  .fg-gate-ti{ font-size: 13px; font-weight: 800; color: #fff; margin-bottom: 6px; }
  .fg-gate-btn { display: block; background: var(--amb); color: #0f1d42 !important; text-decoration: none; padding: 8px 14px; border-radius: 6px; font-size: 11px; font-weight: 800; }

  /* ── SPINNER ──────────────────────────────────────────────── */
  .fg-spin   { width: 15px; height: 15px; border: 2px solid rgba(45,64,128,0.5); border-top-color: var(--amb); border-radius: 50%; animation: spin 0.8s linear infinite; flex-shrink: 0; }
